...
function mergeProducts(remoteProds: Product[], localProds: Product[]): Product[] {
  // Remote (Supabase) is the multi-device source of truth.
  // Local is only used to fill seed catalog items not yet in the DB,
  // and never for unsynced admin-only IDs when remote data is present.
  const map = new Map<string, Product>();
  const names = new Set<string>();

  for (const p of remoteProds) {
    if (isDummyProduct(p)) continue;
    map.set(p.id, p);
    if (p.name) names.add(p.name.toLowerCase());
  }

  const hasRemote = remoteProds.length > 0;
  for (const p of localProds) {
    if (isDummyProduct(p)) continue;
    if (map.has(p.id)) continue;
    if (p.name && names.has(p.name.toLowerCase())) continue;
    // Skip local-only admin drafts when we already have a live remote catalog
    if (hasRemote && (p.id.startsWith('prod-admin-') || p.id.startsWith('custom-'))) continue;
    map.set(p.id, p);
    if (p.name) names.add(p.name.toLowerCase());
  }
  ...
  if (supabase) {
    try {
      ...
      if (error) {
        ...
        // Always merge — even empty remote clears unsynced admin-only locals via mergeProducts
...
/** Resolve a local slug / name / uuid to a Supabase categories.id UUID */
async function resolveCategoryUuid(categoryId: string): Promise<string | null> {
  if (!supabase || !categoryId) return null;
  const target = slugify(categoryId);
  const { data: dbCategories, error } = await supabase.from('categories').select('id, name, parent_id');
  if (error) {
    console.error('resolveCategoryUuid fetch error:', error);
    return null;
  }
  ...
  // Create missing category by humanized name
  const niceName = categoryId
    .replace(/-/g, ' ')
    .replace(/\b\w/g, (ch) => ch.toUpperCase());
  const { data: created, error: createErr } = await supabase
    .from('categories')
    .insert([{ name: niceName, image_url: 'https://images.unsplash.com/photo-1513201099705-a9746e1e201f?auto=format&fit=crop&q=80&w=800' }])
    .select('id')
    .single();
  if (createErr) {
    console.error('resolveCategoryUuid create error:', createErr);
    return null;
  }
...
export async function createProduct(product: {
  ...
  // Prefer real image URLs; drop empty strings
  const imageUrls = (product.images || []).filter((u) => typeof u === 'string' && u.trim().length > 0);
  if (imageUrls.length === 0) {
    imageUrls.push('https://images.unsplash.com/photo-1513201099705-a9746e1e201f?auto=format&fit=crop&q=80&w=800');
  }

  // --- Supabase is required for multi-device visibility ---
  if (!supabase) {
    throw new Error(
      'Supabase is not configured. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in Vercel Project Settings → Environment Variables, then redeploy.'
    );
  }

  const dbCategoryUuid = await resolveCategoryUuid(product.category_id);
  if (!dbCategoryUuid) {
    throw new Error(
      `Could not resolve category "${product.category_id}" in the database. Create the category first in Admin → Categories.`
    );
  }
  ...
  const payload = {
    ...
    // omit created_by — column is UUID FK to auth.users; mock admin has no real user id
  };

  const { data, error } = await supabase
    .from('products')
    .insert([payload])
    .select(`*, categories (id, name, parent_id), product_images (*)`)
    .single();

  if (error || !data) {
    console.error('Supabase createProduct error:', error);
    throw new Error(error?.message || 'Failed to save product to the database.');
  }

  const prod = data as Product;

  // Attach images (best-effort; product row already saved)
  if (imageUrls.length > 0) {
  ...
  const fullProd = (await getProductById(prod.id)) || {
    ...prod,
    product_images: imageUrls.map((url, i) => ({
      id: `img-${prod.id}-${i}`,
      product_id: prod.id,
      image_url: url,
      is_featured: i === 0,
  ...
  // Mirror into local cache so this browser updates instantly
  const localProds = getLocalProducts().filter((p) => p.name.toLowerCase() !== product.name.toLowerCase());
  localProds.unshift(fullProd);
...
export async function updateProduct(id: string, updates: Partial<Product> & { images?: string[] }) {
  ...
  const { images, categories, product_images, created_by, ...rawUpdates } = updates as any;

  // Resolve category slug → UUID if needed
  if (rawUpdates.category_id) {
    const uuid = await resolveCategoryUuid(String(rawUpdates.category_id));
    if (uuid) rawUpdates.category_id = uuid;
  }

  // Recompute discount when prices change
  if (rawUpdates.price !== undefined || rawUpdates.original_price !== undefined) {
    const current = await getProductById(id);
    const price = rawUpdates.price !== undefined ? rawUpdates.price : current?.price ?? 0;
    const originalPrice =
      rawUpdates.original_price !== undefined ? rawUpdates.original_price : current?.original_price;
    rawUpdates.discount_percentage =
  ...
  if (images && Array.isArray(images) && images.length > 0) {
    // Replace images: delete existing then insert
  ...
  const fullProd = (await getProductById(id)) || (data as Product);

  // Update local cache
  const localProds = getLocalProducts();
  const idx = localProds.findIndex((p) => p.id === id);
  if (idx !== -1) {
    localProds[idx] = fullProd;
  } else {
    localProds.unshift(fullProd);
  }
  saveLocalProducts(localProds);
...
export async function deleteProduct(id: string) {
  ...
  // Delete images first (FK), then product
  const { error: imgErr } = await supabase.from('product_images').delete().eq('product_id', id);
  if (imgErr) console.warn('deleteProduct images warning:', imgErr);

  const { error } = await supabase.from('products').delete().eq('id', id);
  if (error) {
    console.error('Supabase deleteProduct error:', error);
    throw new Error(error.message || 'Failed to delete product from the database.');
  }
